...
- [x] No Rust toolchain — resolved 2026-08-02: rustup (stable 1.97.1) + VS Build Tools 2022 (VCTools workload) installed; Tauri desktop app scaffolded in `frontend/src-tauri/`. `cargo check` and release build verified.
- [ ] Tauri desktop app: the packaged app requires the FastAPI backend running locally (`uv run uvicorn backend.api.main:app`) — no bundling of the Python backend; dev mode proxies through Vite. Icons are the Tauri defaults (custom branding pending).
